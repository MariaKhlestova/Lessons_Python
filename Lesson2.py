...
sum_z = 0
for i in range(12):
    zarplata = int(input("Введите зарплату за каждый месяц: "))
    if zarplata > 0:
        sum_z += zarplata
    else:
...
# Введите число карточек — N. Затем введите N − 1 номера оставшихся карточек. 
...
for i in range(1, N + 1):
    sum += i
for i in range(N - 1):
    lost_N = int(input("Введите номер оставшейся карты: "))
    sum -= lost_N
print("Номер потерянной карты ", sum)
"""
# Задача 6: Кинотеатр
...
if (boys > 2 * girls) or (girls > 2 * boys):
    ...
    for i in range(count):
        result += 'i'
    for j in range(girls - count):
        result += 'j'
    else:
        count = girls - boys
    for h in range(count):
        result += 'h'
    for m in range(boys - count):
        result += 'm'
print(result)
"""